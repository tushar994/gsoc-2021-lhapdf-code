...
double interpolate_slope_app(double x, double* values, double* x_val){
    double a[4];
    // cout<<"values are (2):\n";
    for(int i=0;i<4;i++){
        a[i] = values[i];
    ...
    dy1 = (a[1] - a[0]) / (x_val[1] - x_val[0]);


    
    dy2 = (a[3] - a[2]) / (x_val[3] - x_val[2]);
...
double interpolate(double x, double q, int flavour){
    ...
    if(x_a[0]==x_a[1]||x_a[2]==x_a[3]){
        ...
        for(int i=0;i<4;i++){
            ...
            values[i] = interpolate_slope_app(x,vals,x_vals);
            // cout<<values[i]<<" ";
        }
    }
    ...
    if(q_a[0]==q_a[1]||q_a[2]==q_a[3]){
        ...
        return interpolate_slope_app(q,values,q_vals);
    }
...
int main(){
    ...
    ofstream MyFile("./vague_analysis/slope-edge.csv");
    // ofstream MyFile("./dense_data/slope-edge.csv");
